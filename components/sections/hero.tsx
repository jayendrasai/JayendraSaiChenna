...
export function Hero() {
  ...
  return <section id="home" ref={heroRef} className="section-anchor relative isolate min-h-[100svh] overflow-hidden border-b border-border pt-28 sm:pt-36 lg:pt-40"><AmbientField /><div className="page-shell relative flex min-h-[calc(100svh-7rem)] flex-col justify-center pb-14 sm:pb-20"><motion.div variants={staggerContainer(.1, .12)} initial={reducedMotion ? false : "hidden"} animate="visible">
    ...
    <motion.div variants={heroVariants} className="mt-10 max-w-3xl"><p className="text-[clamp(1.75rem,3.8vw,3.5rem)] font-medium leading-[.98] tracking-tight">{siteConfig.subtitle}</p><p className="mt-6 max-w-2xl text-base leading-7 text-foreground/60 sm:text-lg">{siteConfig.heroSubheadline}</p></motion.div>
  ...
  </motion.div><div className="mt-auto grid gap-5 border-t border-border pt-4 text-sm text-foreground/55 sm:grid-cols-3"><div><span className="mb-2 block font-mono text-[.6875rem] uppercase tracking-[.14em] text-muted">{siteContent.hero.focusLabel}</span>{siteContent.hero.focusValue}</div><div><span className="mb-2 block font-mono text-[.6875rem] uppercase tracking-[.14em] text-muted">{siteContent.hero.locationLabel}</span>{siteConfig.location}</div><div><span className="mb-2 block font-mono text-[.6875rem] uppercase tracking-[.14em] text-muted">{siteContent.hero.scrollLabel}</span><span className="inline-flex items-center gap-2">{siteContent.hero.scrollValue} <span className="h-px w-8 bg-accent" /></span></div></div></div></section>;
}
