...
export function ProjectStage({ project }: { project: Project }) {
  return <div className={`project-stage project-stage--${project.stage} relative min-h-[20rem] overflow-hidden p-5 sm:min-h-[25rem] sm:p-8`} aria-label={`${project.title} architecture visual`}>
    <div className="absolute left-5 top-5 font-mono text-[.625rem] uppercase tracking-[.16em] text-black/45 sm:left-8 sm:top-8">{project.eyebrow}</div>
...
function EvaluationStage() {
  return <div className="flex h-full flex-col justify-center gap-5 pt-8"><div className="flex items-end justify-between"><span className="font-mono text-xs uppercase tracking-[.14em] text-black/45">MODEL BENCHMARK</span><span className="font-mono text-xs text-black/45">v.04 / CI</span></div><div className="grid grid-cols-5 gap-1">{[".88", ".91", ".76", ".83", ".94", ".79", ".86", ".72", ".89", ".81", ".92", ".84", ".78", ".95", ".87"].map((value, index) => <div key={`${value}-${index}`} className={`stage-score ${index % 5 === 3 ? "stage-score--hot" : ""}`}>{value}</div>)}</div><div className="flex justify-between border-t border-black/15 pt-3 font-mono text-[.625rem] uppercase tracking-[.12em] text-black/45"><span>faithfulness</span><span>relevancy</span><span>bertscore</span></div></div>;
}

function FeatureStoreStage() {
  return <div className="flex h-full flex-col justify-center gap-5 pt-8"><div className="flex items-center gap-2"><span className="stage-pulse" /><span className="font-mono text-xs uppercase tracking-[.14em] text-black/55">EVENT STREAM / LIVE</span></div><div className="stage-stream"><span /><span /><span /><span /><span /></div><div className="grid grid-cols-2 gap-3"><div className="stage-storage stage-storage--hot"><strong>REDIS</strong><small>online serving</small><b>12 ms</b></div><div className="stage-storage"><strong>POSTGRESQL</strong><small>durable history</small><b>source of truth</b></div></div></div>;
}
